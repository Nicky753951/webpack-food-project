...
function slider({
  ...
  function deleteNot(str) {
    return +str.replace(/\D/g, '');
  }

  function setOpacity(element, index) {
    return element.forEach(element => element.style.opacity = "0.5"), element[index - 1].style.opacity = "1";
  }
  ...
  next.addEventListener('click', () => {
    if (offset == deleteNot(width) * (slides.length - 1)) {
      offset = 0;
    } else {
      offset += deleteNot(width);
    }
    ...
    setOpacity(dots, slideIndex);
  });
  prev.addEventListener('click', () => {
    if (offset == 0) {
      offset = deleteNot(width) * (slides.length - 1);
    } else {
      offset -= deleteNot(width);
    }
...
window.addEventListener('DOMContentLoaded', function () {
  const modalTimerId = setTimeout(() => (0,_modules_modal__WEBPACK_IMPORTED_MODULE_0__.default)('.modal', modalTimerId), 300000);
